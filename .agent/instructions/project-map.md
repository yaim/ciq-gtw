...
**Still planned:** the rest of the boundaries below — `src/api/` chat-completions routing, the rest of `src/openai/` (request schemas, normalization, response/SSE encoders), the rest of `src/generation/` (orchestration, polling, capacity, cancellation), `src/prompts/`, and `src/tools/`. The boundary ownership rules below apply as those modules are created.
