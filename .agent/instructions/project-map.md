...
- `src/idempotency/` is the ONLY code allowed to know Redis exists, to hold the derived idempotency subkeys, or to know the record/payload wire formats. It exposes a narrow port (`store.ts`) plus a coordinator that returns discriminated outcomes.
- It depends only on normalized values and generic ports (`Clock`, `Sleeper`, `RandomFn`); it must not import from `src/api/` or take a Fastify request/reply. Public statuses, envelopes, and headers stay owned by the API boundary.
