...
Read `.agent/docs/tech-software-spec.md` sections 19, 19.2, 21–23, 24, 31, 33, and 34 before security-sensitive changes. Prompt/tool work also requires the tool-calling guide; upstream work requires the upstream guide.

## Implemented Foundation Controls

These controls exist today and must be preserved:

- **Gateway client authentication** — `src/api/gateway-auth.ts` authenticates every `/v1/*` route (`src/api/v1-routes.ts` route group) via `Authorization: Bearer <gateway-key>` against `COLLECTIVIQ_GATEWAY_KEYS`; `/healthz` and `/readyz` stay unauthenticated. The scheme is case-insensitive, the token is compared **exactly** (no trim/normalize), and comparison is fixed-length: configured keys become SHA-256 digests once, the presented token is hashed once, and each digest is checked with `node:crypto` `timingSafeEqual` **without** an early return on a match. Bounds (`GATEWAY_KEY_LIMITS` in `src/config/schema.ts`): ≤64 keys, ≤8192 UTF-8 bytes/key, with the same byte cap bounding a presented token before hashing. Missing/malformed/empty/oversized/incorrect credentials all return one fixed OpenAI `401`; authentication is mandatory (no disable switch); the gateway key is never forwarded upstream, logged, or reflected. On a successful match the route records only **opaque** per-configured-key identities — never the raw key or its digest: `k<index>` for the process-local per-key capacity accounting, plus (when the corresponding optional feature is configured) the cross-replica idempotency scope, the cross-replica rate-limit scope (Phase 4B), the cross-replica thread-reuse scope (Phase 5A), and the cross-replica CAPACITY scope `capacityScopeId` (Phase 4D) — FIVE identities in total, of which four are cross-replica scopes. All five are precomputed once at authenticator construction so the raw key is never re-read per request; each cross-replica scope is derived under its OWN HKDF domain, so a single key's four scopes are four unrelated values; each is `null` when its feature is disabled; and none is ever logged, reflected, or returned. `k<index>` is deliberately never written to Redis — it is positional, so it would leak the configured key ORDER and could not be shared across replicas anyway. Public errors come only from the shared bounded OpenAI envelopes in `src/openai/errors.ts` (the full Phase 1B set: `400`/`401`/`404`/`413`/`429`/`500`/`502`/`503`/`504`); the fixed `500` and the completion route's fail-closed boundary (below) never inspect, serialize, log, or reflect the thrown value. Keep this separate from the upstream credential provider (`auth.ts`).
...
- the optional cross-replica active-permit registry (`SHARED_CAPACITY_ENABLED`) — its member size, member count, claim-batch size, and lease ceiling;
...
- Redis is optional and limited to short-lived idempotency/status/final-response state, one bounded per-scope rate-limit timestamp when Phase 4B is enabled, one bounded encrypted session-to-thread mapping when Phase 5A thread reuse is enabled, and one bounded namespace-level ACTIVE-permit lease registry when Phase 4D shared capacity is enabled. It holds no queue state, and no capacity state at all unless Phase 4D is explicitly enabled — with that feature off (the default), capacity is entirely process-local.
...
`src/idempotency/` owns every idempotency record written to Redis (it is one of
exactly four features permitted to store state there, and it writes nothing
outside its own `:idem:` keyspace). Specification section
...
- **Key derivation.** One 32-byte master key is expanded with HKDF-SHA-256 into
  ...
  kept separate from the process-local per-key capacity identity `k<index>`.
...
`src/rate-limit/` is one of the four features allowed to know its
...
- **What Redis holds.** One bounded decimal integer of microseconds (a
  ...
  domain-separated from the `:idem:`, `:capacity:`, and `:reuse:` keyspaces.
  Never a raw gateway key, the process-local `k<index>`, an authorization value,
  a prompt, a request body, a model id, a thread id, or completion content.
  Neither the scope nor the storage key is ever logged or reflected.
...
- **Residual risks to state honestly.** Enforcement is bounded to the configured
  ...
  key's allowance to a full burst undetectably; capacity accounting is
  process-local unless the separate, off-by-default Phase 4D layer is enabled
  (this feature neither depends on it nor changes with it); and all replicas must
  share the endpoint, key, prefix, gateway-key set, and `RATE_LIMIT_*` settings.

**Implementation status (Phase 4D, implemented — OPTIONAL, off by default).**
`src/shared-capacity/` is the fourth and last feature permitted to write Redis
state, and it writes nothing outside its single `:capacity:` registry key.
Specification section 19.2 owns the normative contract; the security-relevant
guarantees are:

- **Off unless explicitly enabled, and fail-closed when on.** With
  `SHARED_CAPACITY_ENABLED=false` (the default) no capacity scope is derived, no
  coordinator is built, and no Redis capacity operation runs. When enabled, a
  disconnected Redis, a command timeout, corrupt or over-cardinality registry
  state, an unusable or ambiguous reply, or a coordinator wired without a derived
  scope all return `503 capacity_unavailable` + `Retry-After: 2`. The gateway
  NEVER admits a request whose cluster occupancy is unknown, because doing so
  would silently multiply the configured cluster-wide limit by the replica count.
- **No new secret.** Enabling it requires `REDIS_URL` (already secret-bearing and
  redacted) and reuses the existing `IDEMPOTENCY_ENCRYPTION_KEY`. The one added
  setting, `SHARED_CAPACITY_ENABLED`, is a non-secret strict boolean, and its
  configuration errors stay value-free. No new dependency and no lockfile change.
- **Separate key domain.** The capacity HMAC subkey is expanded from the same
  master key under a DISTINCT HKDF salt and `info` label with its own domain
  tags, so it is cryptographically independent of every idempotency, rate-limit,
  and thread-reuse subkey, and a gateway key's four cross-replica scopes are four
  unrelated values. No code is shared with the other three keyrings — the
  length-framing helper is deliberately duplicated — so a change here can never
  re-key another feature's stored state. Derivation is deterministic across
  replicas and independent of `COLLECTIVIQ_GATEWAY_KEYS` ORDER, which is exactly
  what lets one active-permit budget span replicas; the positional process-local
  `k<index>` is never written to Redis.
- **A FIFTH opaque gateway-key identity.** `capacityScopeId` is computed once per
  configured key at authenticator construction, so the raw key is never re-read
  per request. It is `null` while the feature is disabled, and neither it, the
  random owner token, nor the derived registry key is ever logged, reflected,
  returned in a response, or placed in an error.
- **What Redis holds.** ONE namespace-level sorted set at
  `<REDIS_KEY_PREFIX>:capacity:<HMAC digest>` whose members are exactly
  `<version>|<owner token>|<capacity scope>` — a format version, 128 bits of
  CSPRNG owner token, and the opaque per-key scope, both unpadded base64url with
  an out-of-alphabet delimiter so no component value can forge an extra field —
  scored by an integer lease deadline stamped from Redis's own `TIME`. It holds
  **no** request, thread, session, model, tool, prompt, answer, credential,
  authorization value, raw gateway key, or process-local `k<index>` identity, and
  nothing that needs encrypting. There is deliberately no per-scope key: the
  global limit has to be counted across all gateway keys, and per-key occupancy
  is counted from the scope inside each member instead.
- **Untrusted-state safety.** Cardinality is checked BEFORE any member is
  materialized, so an over-cardinality registry is classified corrupt without its
  bytes being read; member size, encoding, score, and owner-token uniqueness are
  all validated server-side before any mutation. A wrong type, an oversized or
  malformed member, a bad score, or a duplicated token fails CLOSED and leaves the
  registry byte-for-byte untouched — the gateway never prunes or repairs it,
  because a "repair" would hand out permits another replica may still hold. Both
  operations are single atomic Lua scripts against Redis `TIME`, never a Node
  clock, and both are total (a closed result union, never a thrown value).
- **No compensation, ever.** An unavailable, corrupt, or ambiguous claim is never
  retried and never compensated: the gateway cannot know whether the mutation
  applied, so a retry could double-count a permit it already holds and a
  speculative release could remove one another replica now holds. Only the
  well-formed "cluster is currently full" outcome is retried, under a bounded
  jittered backoff, and that outcome leaves the waiter QUEUED rather than
  rejecting it.
- **Content-free public errors.** The `503 capacity_unavailable` body and the
  existing `429 gateway_capacity_exceeded` body are fixed and reveal no limit,
  occupancy, scope, owner token, or registry key. A request rejected here creates
  no thread, submits nothing, registers no title correlation, and makes no
  upstream call; on the streamed transport it arrives as a content-free SSE error
  record because capacity is reached after the SSE headers by design.
- **Residual risks to state honestly.** `maxmemory-policy noeviction` is
  MANDATORY here (the Phase 4A requirement applies unchanged): an evicted or lost
  registry key forgets live permits and can briefly OVER-admit up to the full
  configured limit again, and the gateway cannot detect it. A Redis restart has
  the same effect. In the other direction, a failed or unacknowledged release
  conservatively UNDER-admits that replica until the lease expires — it never
  changes an already successful response — and an ambiguous claim can leave an
  orphan lease that the per-instance capacity gauges cannot see. The lease is a
  crash reaper with no heartbeat and no renewal, so a hard-killed replica's
  permits are reclaimed only when their deadlines pass. Enabling the feature
  deliberately trades availability for correctness on the completion path, and
  mixed local/shared rolling operation is unsupported: enablement or a capacity-
  limit change needs a coordinated drain/restart, and every replica must share
  the endpoint, key, prefix, gateway-key set, `SHARED_CAPACITY_ENABLED` value,
  and capacity limits. The real-Redis suite for this feature
  ...
  specification section 29.10) — which proves the Lua and store contract against
  Redis 8.8.2 in that configuration, not production readiness or cross-version
  behaviour. The Phase 4 load and security gates remain outstanding.

**Implementation status (Phase 5A, implemented — OPTIONAL, off by default).**
`src/thread-reuse/` owns the session-to-thread mapping, one of the four
features permitted to write Redis state, and it writes nothing outside its own
...
- **Off unless configured.** `OPENCODE_THREAD_REUSE_ENABLED=false` (the default)
  ...
  every Phase 4A, Phase 4B, and Phase 4D domain, with no shared code, so a change
  here can never re-key existing records.
...
- **Residual risks to state honestly.** It is NOT production ready (the remaining
  Phase 4 controls — load testing, the broader security review, dependency
  scanning, and backup/release procedures — are outstanding); upstream ordering,
  pagination, cleanup, and retention are unverified; a post-`create_thread` Redis
  failure can leave one blank orphan thread that is deliberately never
  guess-deleted; a hard replica kill mid-submit
